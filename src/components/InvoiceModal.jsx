              ...
              <div className="p-4" id="print">
                ...
                <div className="mt-6">
                  <div className="grid grid-cols-2 gap-x-6 gap-y-4">
                    ...
                    <span className="font-bold">Cashier Address:</span>
                    <span>{invoiceInfo.cashierAddress}</span>
                  ...
                  <div className="mt-4 flex flex-col items-end space-y-2">
                    <div className="flex w-full justify-between border-t border-black/10 pt-2">
                      ...
                      <span>
                      {currencyList.find((currency) => currency.abbreviation === selectedCurrency)?.symbol}
                        {invoiceInfo.convertedSubtotal.toFixed(2)}</span>
                    </div>
                    <div className="flex w-full justify-between">
                      <span className="font-bold">Discount:</span>
                      <span>{currencyList.find((currency) => currency.abbreviation === selectedCurrency)?.symbol}
                        {invoiceInfo.convertedDiscountRate.toFixed(2)}</span>
                    </div>
                    <div className="flex w-full justify-between">
                      <span className="font-bold">Tax:</span>
                      <span> {currencyList.find((currency) => currency.abbreviation === selectedCurrency)?.symbol}
                        {invoiceInfo.convertedTaxRate.toFixed(2)}</span>
                    </div>
                    <div className="flex w-full justify-between border-t border-black/10 py-2">
                      <span className="font-bold">Total:</span>
                      <span className="font-bold">
                     {currencyList.find((currency) => currency.abbreviation === selectedCurrency)?.symbol}
                        {invoiceInfo.convertedTotal % 1 === 0
                          ? invoiceInfo.convertedTotal
                          : invoiceInfo.convertedTotal.toFixed(2)}
                      </span>
                    </div>
                  </div>
                </div>
